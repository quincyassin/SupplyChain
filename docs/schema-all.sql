...
-- 导入分单订单明细（物理归档）
CREATE TABLE IF NOT EXISTS import_order_archive (
    system_no VARCHAR(20) NOT NULL PRIMARY KEY COMMENT '系统编号（10位雪花ID，主键）',
    task_id BIGINT NOT NULL COMMENT '分单任务ID，关联 process_task.id',
    merchant VARCHAR(128) NOT NULL COMMENT '商家',
    ...
    source_row_num INT COMMENT 'Excel原始行号',
    created_at DATETIME NOT NULL COMMENT '入库时间',
    INDEX idx_import_order_archive_task_id (task_id),
    INDEX idx_import_order_archive_merchant (merchant),
    INDEX idx_import_order_archive_platform (platform),
    INDEX idx_import_order_archive_issue_date (issue_date),
    INDEX idx_import_order_archive_receipt_status (receipt_status),
    INDEX idx_import_order_archive_after_sales (after_sales),
    INDEX idx_import_order_archive_after_sales_status (after_sales_status)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='导入分单订单明细（物理归档）';
