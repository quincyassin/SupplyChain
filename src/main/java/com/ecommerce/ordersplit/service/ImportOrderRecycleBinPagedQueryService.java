 ...
 */
@Service
public class ImportOrderRecycleBinPagedQueryService {
    ...
    public long countOrders(ImportOrderListFilter filter) {
        StringBuilder sql =
                new StringBuilder("SELECT COUNT(*) FROM import_order_recycle_bin o WHERE 1=1");
    ...
    public List<ImportOrderRecycleBin> findAllOrders(ImportOrderListFilter filter) {
        StringBuilder sql =
                new StringBuilder("SELECT o.* FROM import_order_recycle_bin o WHERE 1=1");
        Map<String, Object> params = new HashMap<>();
        appendFilters(sql, params, filter);
        sql.append(" ORDER BY o.deleted_at DESC, o.system_no DESC");

        Query query = entityManager.createNativeQuery(sql.toString(), ImportOrderRecycleBin.class);
        bindParams(query, params);
    ...
    public List<MerchantSplitGroupDto> summarizeByMerchant(ImportOrderListFilter filter) {
        StringBuilder sql =
                new StringBuilder(
                        ...
                        FROM import_order_recycle_bin o WHERE 1=1""");
        Map<String, Object> params = new HashMap<>();
        appendFilters(sql, params, filter);
        ...
        StringBuilder sql =
                new StringBuilder(
                        ...
                        SELECT
                          ...
                          COUNT(*) AS row_count,
                          SUM(CASE WHEN o.receipt_status = 'RECEIPTED' THEN 1 ELSE 0 END) AS receipted_count
                        FROM import_order_recycle_bin o
