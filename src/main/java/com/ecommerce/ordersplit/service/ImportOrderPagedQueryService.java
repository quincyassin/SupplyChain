 ...
 */
@Service
public class ImportOrderPagedQueryService {
    ...
    public long countOrders(ImportOrderListFilter filter) {
        StringBuilder sql = new StringBuilder("SELECT COUNT(*) FROM import_order o WHERE 1=1");
    ...
    public List<ImportOrder> findAllOrders(ImportOrderListFilter filter) {
    ...
    public List<PlatformSummaryDto> summarizeByPlatform(
        ...
        for (Object[] row : rows) {
            String platform = row[0] == null ? ImportOrderQueryService.UNKNOWN_PLATFORM : row[0].toString();
            int rowCount = row[1] == null ? 0 : ((Number) row[1]).intValue();
            int receiptedCount = row[2] == null ? 0 : ((Number) row[2]).intValue();
